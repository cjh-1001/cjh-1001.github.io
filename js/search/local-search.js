...
class LocalSearch {
  ...
  getResultItems (keywords) {
    ...
    this.datas.forEach(({ title, content, url }) => {
      ...
      // If no content matches, use the first part of the content as summary
      if (slicesOfContent.length === 0 && content.length > 0) {
        const start = 0
        const end = Math.min(content.length, 120)
        slicesOfContent.push({ hits: [], start, end, count: 0 })
      }
